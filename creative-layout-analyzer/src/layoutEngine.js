class LayoutEngine {
  ...
  findOptimalSubhead(elementConfig) {
    ...
    for (const size of elementConfig.ratios) {
      let subheadCol;
      ...
      for (let row = headerBottom; row <= this.gridSize - size.h - startPos; row++) {
        ...
        const score = analysis.averageScore * analysis.uniformityScore * distancePenalty;
    ...
    candidates.sort((a, b) => {
      if (a.distanceFromHeader !== b.distanceFromHeader) {
